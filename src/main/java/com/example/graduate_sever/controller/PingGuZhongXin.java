...
//        System.out.println(chanXueYanDTO.getKey()+"______"+chanXueYanDTO.getPageIndex()+"_______"+chanXueYanDTO.getPageSize());
    ...
    public JsonBean updatePingGuZhongXin(@RequestBody PingGuZhongXinXiangGuanUO uo){
        ...
        int role=uo.getRole();
        if (role!=4&&role!=1){
            element=new PingGuZhongXinXiangGuanEntity(0,uo.getFinishtime(),uo.getGrade(),uo.getPartment(),uo.getName());
        }else {
            element=new PingGuZhongXinXiangGuanEntity(1,uo.getFinishtime(),uo.getGrade(),uo.getPartment(),uo.getName());
        }

        return  pingGuZhongXinService.insertPingGuZhongXin(element);
    }
