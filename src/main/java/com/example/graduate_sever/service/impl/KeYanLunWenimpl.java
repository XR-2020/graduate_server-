...
//                System.out.println("id="+entity[i]);
                HeBingEntity keYanLunWenEntity=new HeBingEntity(1,parent[i+2],parent[i+7],parent[i+1]);
                mapper.insertKeYanLunWen(keYanLunWenEntity);
                //添加第一完成人
                mapper.insertKeYanLunWenParticipation(new ParticipationEntity(Integer.parseInt(parent[i+6]),keYanLunWenEntity.getId(),10));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
